...
as possible. If you developed your deployment based on this example, then
...
breaking your deployment. The main issues to keep an eye on:

- This guide was written for commit [eda40d0](https://github.com/immich-app/immich-charts/tree/eda40d0d059d3e211ebe0dd5a32f2b8ac0cd68cc).
...
- The Valkey (cache) deployment will be completely replaced. This shouldn't
  ...
  quiet period (or stopping Immich entirely) so there are no outstanding jobs
...
- The example sets `server.configuration = null` to mimic the original example,
  ...
  the URL to the include machine learning service. The official chart uses the
  `IMMICH_MACHINE_LEARNING_URL` environment variable which has been deprecated.
...
- Versions might change: The example does not hardcode any values since they'll
  ...
  changes, set those to match exactly your current version. Version upgrades
...
the changes that would be made by applying replacing the Helm chart. We
...
Adjust the values for namespace and release to your environment. The
`--normalize-manifests` option provides a cleaner diff that's easier to review.
There will be quite a few expected changes:
...
the migration. If there are differences outside of the above, updated your
...
so make nobody needs to access Immich right now.
